...
Methods for dealing with imbalanced classes typically fall under:
- Data resampling: under/over-sampling
- Cost-sensitive classification: penalizing our model for misclassifying certain categories.

#### Resampling
The basic idea behind resampling is to adjust our dataset in a way that **removes** the class imbalance we experience in the data. This can be done using either oversampling or undersampling. 

Undersampling aims to reduce the occurence of the majority class in our data. This method removes records which belong to the majority class and can be done in several different ways. We can remove these records at random, use clustering to select the majority samples or use the *Tomek Links* method. 

The Tomek Links method involves using clustering to remove the points belonging to the majority class which are *close* to the points in the minority class. Closeness is assessed using a distance metric such as the Euclidean distance. 
...
Given some transactions we may wish to find rules which allow us to predict occurrence of an item given the occurrence of other items. For example we want an output such as $\{Diaper\} \rightarrow \{Beer\}$ indicating that customers who buy diapers also purchase beer. Clearly this information is very valuable for retail stores who could use this information to re-position products in their physical stores or on their websites. 
Note that this example comes from an urban myth about an unexpected correlation, between sales of diapers and beer, that was discovered by a Walmart employee. 

The main difficulty in association rule mining (ARM) comes from the large number of possible association rules. If we limited our search to only look for rules concerning two products the total number of association rules would still be $2^n$. This means that the total number of rules grows at an exponential rate. For businesses which sell 1000's of products it is clear that mining all these rules may not be computationally feasible and is definitely not efficient! 

Therefore we need to use algorithms which enable us to limit our search space. 
...
\
OF: **What is clustering and why is it useful?** Clustering analysis is a form of unsupervised learning which focuses around splitting data into groups. Clustering uses distance measures in order to group similar observations into clusters.
...
**DBSCAN**
 ...
 - More hyperparameters to specify, makes tuning more difficult
