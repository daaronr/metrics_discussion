...
[@osterUnobservableSelectionCoefficient2019].
...
Whilst they are very simple and intuitive to understand they form the
...
be used for regression settings.

A decision tree works using the idea of *recursive binary splitting*.
Essentially this means that data is split into subsets which allow the
model to predict observations well. <!-- Guess who example --> A
...
calculate **information gain** or **Gini impurity**. As these two
measures are essentially trying to calculate the same thing we will only
discuss information gain.
...
Using entropy we are able to compute information gain. With the
...
relationships between $X$ and $Y$. These models have very low bias,
however, they do have high variance. This means that changes in the
training data may cause large changes in the predictions made by the
model.
...
sub-samples of our data. The algorithm works as follows:
...
2.  Use this sample to fit a tree, $T_b$:
    ...
    i.  Select $m$ variables from random out of a total of $p$ variables

    ii. Pick the best variable to split the data on out of $m$

    iii. Split the node

    This is repeated until the minimum node size is reached.
