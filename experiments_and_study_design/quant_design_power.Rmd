...
BB: further notes added 17/06/2020
This isn't really on the baove discussion of the value of pre-reg, but on a topic that i think is at least somehwat related. Namely that when we have multiple hypotrheses tetsing in a paper how do we control for that and what do our p-values really mean. Particualry, there is [this paper](https://academic.oup.com/qje/article/134/2/557/5195544) by Alwyn Young which (I think) is basically saying that when we have a load of hypotheses within a study and then one has a significant p value we are quite a lot of the time pciking up a false result. obviously that isn't very much of a surprise. But i think it also talks somewhat to the Steffano DellaVigno paper that was rpesented at the VAFE - that the publication bias really creeps in when we are publishing studies that have just one or two p values around 0.05. After all, it is those studies which have a real pay-off from getting that significant result.
