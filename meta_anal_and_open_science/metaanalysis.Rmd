...
FE model calculates weighted average of the effect sizes for the pooled effect size:

$$\hat{\theta}_F = \frac{\sum\limits_{k=1}^K \hat{\theta_k}/\hat{\sigma}^2_k}{\sum\limits_{k=1}^K 1/\hat{\sigma}^2_k} $$
\
...
<div class="marginnote">
Not sure I understand this; may need to revise and give a clear explanation. I guess it's something like 'the estimated parameter densities can then be drawn and run to simulate a dgp, which should yield data looking like the original data set'...
</div>
